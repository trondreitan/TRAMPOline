...
init.params.flat=function(hyp)
{
  ...
  ret=list(
   ...
   o18.optim=rnorm(N.sp,hyper$o18.optim.mu,hyper$o18.optim.sd/4),
   o18.lwidth=rnorm(N.sp,hyper$o18.lwidth.mu+7,hyper$o18.lwidth.sd/4),
   f.occu=rep(0,Nf),
   f.bin=rep(0,Nf),
...
logprior.flat=function(par, hyp)
{
  ...
  o18.optim=par[(i+1):(i+N.sp)]; i=i+N.sp
  o18.lwidth=par[(i+1):(i+N.sp)]; i=i+N.sp
  ...
  sum(dnorm(o18.optim, hyper$o18.optim.mu, hyper$o18.optim.sd, log=T)) +
  sum(dnorm(o18.lwidth, hyper$o18.lwidth.mu, hyper$o18.lwidth.sd, log=T))
}
...
# need to send it, really...
{
  ...
  occu.mat[,(1:(N.sp-1))]=ilogit(const.occu.mat+sf.occu[Y$form.nr,] +
	...
	(Y$O18.mean%*%t(rep(1,N.sp-1))-rep(1,N)%*%t(o18.optim[1:(N.sp-1)]))^2/
	(rep(1,N)%*%t(o18.width[1:(N.sp-1)]))^2 )

  const.bin.mat=rep(1,N)%*%t(logit.const.bin)
  bin.mat=ilogit(const.bin.mat + f.bin.use[Y$form.nr,] + sf.bin[Y$form.nr,] -
    (Y$O18.mean%*%t(rep(1,N.sp))-rep(1,N)%*%t(o18.optim))^2/
	(rep(1,N)%*%t(o18.width))^2 )
