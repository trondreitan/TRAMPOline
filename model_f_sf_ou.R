...
hyper.this=list(logsd.mu=0, logsd.sd=4, log.betabin.s.mu=0, log.betabin.s.sd=4,
		...
		log.dt.mu=log(0.25), log.dt.sd=log(10)/1.96)
...
init.params.flat=function(hyp)
{
  ...
  ret=list(
   ...
   log.dt.f.occu=rnorm(1, hyper$log.dt.mu, hyper$log.dt.sd),
   log.dt.f.bin=rnorm(1, hyper$log.dt.mu, hyper$log.dt.sd),
   log.dt.sf.occu=rnorm(1, hyper$log.dt.mu, hyper$log.dt.sd),
   log.dt.sf.bin=rnorm(1, hyper$log.dt.mu, hyper$log.dt.sd),
   f.occu=rep(0,Nf),
   f.bin=rep(0,Nf),
...
logprior.flat=function(par, hyp)
{
  ...
  log.dt.f.occu=par[i+1]; i=i+1
  log.dt.f.bin=par[i+1]; i=i+1
  log.dt.sf.occu=par[i+1]; i=i+1
  log.dt.sf.bin=par[i+1]; i=i+1
  ...
  sum(dnorm(log.dt.f.occu, hyper$log.dt.mu, hyper$log.dt.sd, log=T)) +
  sum(dnorm(log.dt.f.bin, hyper$log.dt.mu, hyper$log.dt.sd, log=T)) +
  sum(dnorm(log.dt.sf.occu, hyper$log.dt.mu, hyper$log.dt.sd, log=T)) +
  sum(dnorm(log.dt.sf.bin, hyper$log.dt.mu, hyper$log.dt.sd, log=T)) 
}
...
# need to send it, really...
{
  ...
  log.dt.f.occu=par[i+1]; i=i+1
  dt.f.occu=exp(log.dt.f.occu)
  log.dt.f.bin=par[i+1]; i=i+1
  dt.f.bin=exp(log.dt.f.bin)
  log.dt.sf.occu=par[i+1]; i=i+1
  dt.sf.occu=exp(log.dt.sf.occu)
  log.dt.sf.bin=par[i+1]; i=i+1
  dt.sf.bin=exp(log.dt.sf.bin)
  ...
  ll=ll+dnorm(f.occu[Nf], 0, sd.f.occu, log=T)
  rho=exp(-(K[(Nf-1):1]-K[Nf:2])/dt.f.occu)
  ll=ll+sum(dnorm(f.occu[(Nf-1):1], rho*f.occu[Nf:2], sqrt(1-rho^2)*sd.f.occu, log=T))
  f.occu.use=f.occu%*%t(rep(1,N.sp-1))
  
  ll=ll+dnorm(f.bin[Nf], 0, sd.f.bin, log=T)
  rho=exp(-(K[(Nf-1):1]-K[Nf:2])/dt.f.bin)
  ll=ll+sum(dnorm(f.bin[(Nf-1):1], rho*f.bin[Nf:2], sqrt(1-rho^2)*sd.f.bin, log=T))
  f.bin.use=f.bin%*%t(rep(1,N.sp))

  K.use=K%*%t(rep(1,N.sp))
  
  ll=ll+sum(dnorm(sf.occu[Nf,], 0, sd.sf.occu, log=T))
  rho=exp(-(K.use[(Nf-1):1,1:(N.sp-1)]-K.use[Nf:2,1:(N.sp-1)])/dt.sf.occu)
  s=rep(1,Nf-1)%*%t(sd.sf.occu)
  ll=ll+sum(dnorm(sf.occu[(Nf-1):1,], rho*sf.occu[Nf:2,], sqrt(1-rho^2)*s, log=T))

  ll=ll+sum(dnorm(sf.bin[Nf,], 0, sd.sf.bin, log=T))
  rho=exp(-(K.use[(Nf-1):1,]-K.use[Nf:2,])/dt.sf.bin)
  s=rep(1,Nf-1)%*%t(sd.sf.bin)
  ll=ll+sum(dnorm(sf.bin[(Nf-1):1,], rho*sf.bin[Nf:2,], sqrt(1-rho^2)*s, log=T))
