...
par.sim[const.bin.fi+1:S-1]=logit(c(0.2,0.1,0.1,0.6))
par.sim[const.occu.fi+1:(S-1)-1]=logit(c(0.5,0.9,0.5))
...
par.sim[sf.occu.fi+1:((S-1)*F)-1]=c(seq(-2,2,length.out=F),rep(0,2*F))
par.sim[sf.bin.fi+1:(S*F)-1]=c(rep(0,F),seq(1,-1,length.out=F),rep(0,2*F))
