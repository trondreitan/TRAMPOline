...
init.params.flat=function(hyp)
{
  ...
  ret=list(
   ...
   ca.optim=rnorm(N.sp,hyper$ca.optim.mu,hyper$ca.optim.sd/4),
   ca.lwidth=rnorm(N.sp,hyper$ca.lwidth.mu+7,hyper$ca.lwidth.sd/4),
   f.occu=rep(0,Nf),
   f.bin=rep(0,Nf),
...
logprior.flat=function(par, hyp)
{
  ...
  ca.optim=par[(i+1):(i+N.sp)]; i=i+N.sp
  ca.lwidth=par[(i+1):(i+N.sp)]; i=i+N.sp
  ...
  sum(dnorm(ca.optim, hyper$ca.optim.mu, hyper$ca.optim.sd, log=T)) +
  sum(dnorm(ca.lwidth, hyper$ca.lwidth.mu, hyper$ca.lwidth.sd, log=T))
}
...
# need to send it, really...
{
  ...
  occu.mat[,(1:(N.sp-1))]=ilogit(const.occu.mat+sf.occu[Y$form.nr,] +
	...
	(Y$ca.mean%*%t(rep(1,N.sp-1))-rep(1,N)%*%t(ca.optim[1:(N.sp-1)]))^2/
	(rep(1,N)%*%t(ca.width[1:(N.sp-1)]))^2 )

  const.bin.mat=rep(1,N)%*%t(logit.const.bin)
  bin.mat=ilogit(const.bin.mat + f.bin.use[Y$form.nr,] + sf.bin[Y$form.nr,] -
    (Y$ca.mean%*%t(rep(1,N.sp))-rep(1,N)%*%t(ca.optim))^2/
	(rep(1,N)%*%t(ca.width))^2 )
